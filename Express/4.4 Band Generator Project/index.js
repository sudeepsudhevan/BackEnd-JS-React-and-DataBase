...
app.post("/submit", (req, res) => {
  ...
  const random_adj = adj[Math.floor(Math.random() * adj.length)]
  const random_noun = noun[Math.floor(Math.random() * noun.length)]

  // generated_name = `${adj[Math.floor(Math.random() * adj.length)]} ${noun[Math.floor(Math.random() * noun.length)]}`

  //2. Send the index.ejs as a response and add the adjective and noun to the res.render
  //3. Test to make sure that the random words display in the h1 element in index.ejs
  res.render("index.ejs", { adjective: random_adj, noun: random_noun })
});
